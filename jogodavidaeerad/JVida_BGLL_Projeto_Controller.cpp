...
void tornarVazioAux(int l, int c){
    ...
    for (int i = 0; i < qtdV; i++) {
        ...
        jdvAux[lV][lC].qtdVizVivos = jdvMatriz[lV][lC].qtdVizVivos - 1;
    }  
}
...
int verificarOcupacaoAux(int l, int c){
	int qtdV = jdvAux[l][c].qtdVizinhos;
	int nL, nC;
	
	for(int i = 0; i < qtdV; i++){
		nL = jdvAux[l][c].infoVizinhos[i].linha;
		nC = jdvAux[l][c].infoVizinhos[i].coluna;
...
int reproducao(int l, int c) {
    ...
    int qtdVivos = jdvMatriz[l][c].qtdVizVivos;
...
int sobrevivencia(int l, int c) {
    ...
    if (jdvMatriz[l][c].qtdVizVivos == 2 || jdvMatriz[l][c].qtdVizVivos == 3) {
        inserirCelAux(l,c); // a celula sobrevive, entao nao precisa mudar nada
        return 1;
    } else {
    	morteFaltaComida(l,c);
    	morteSolidao(l,c);
...
int morteFaltaComida(int l,int c){
	...
	if(jdvMatriz[l][c].qtdVizVivos >= 4){
		if(verificarOcupacaoAux(l,c) == 1)
			jdvAux[l][c].situacao = '+';
...
int morteSolidao(int l, int c){
	...
	if(jdvMatriz[l][c].qtdVizVivos == 0){ 
		return 1;// a celula morre por falta de amigos
	} else if(jdvMatriz[l][c].qtdVizVivos == 1){
		jdvAux[l][c].situacao = '+';
		return 1;
	}
}
...
void copiarMatrizAux(int dim) {
    ...
    for (int i = 0; i < dim; i++) {
        for (int j = 0; j < dim; j++) {
            ...
            for (int k = 0; k < jdvAux[i][j].qtdVizinhos; k++) {
            	jdvMatriz[i][j].infoVizinhos[k] = jdvAux[i][j].infoVizinhos[k];
        	}
